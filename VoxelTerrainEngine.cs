			...
			if(ActiveChunks.Count>0){
				...
				{	
					...
					if(i< ActiveChunks.Count && ActiveChunks[i]!=null &&  ActiveChunks[i].canDraw && ActiveChunks[i].shouldrender){
						...
						if(Vector2.Dot(new Vector2(direction.x,direction.y),new Vector2(target.forward.x,target.forward.z) )>=number|| Physics.Raycast(playerGlobal,Vector3.down,out hit,5 ,mask)){
